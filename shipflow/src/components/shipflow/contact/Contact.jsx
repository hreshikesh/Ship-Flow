      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            {/* Description — real SHIPFLOW positioning */}
            <p className="mt-6 max-w-xl text-base leading-relaxed text-slate-400 sm:text-lg">
              SHIPFLOW is a specialized CFD software suite for resistance,
              propulsion, and seakeeping predictions. Built by naval architects,
              for naval architects — delivering results in hours, not weeks.
            </p>

            {/* Feature highlights — real SHIPFLOW modules */}
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {[
                { icon: Waves, label: "XPAN", sub: "Potential Flow" },
                { icon: Gauge, label: "XCHAP", sub: "RANS Solver" },
                { icon: Zap, label: "XBOUND", sub: "Boundary Layer" },
              ].map((mod) => (
                <div
                  key={mod.label}
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
              ...
              >
                ...
                <span className="relative">Request a Quotation</span>
                <ArrowRight
                  size={18}
            ...
            <div className="mt-8 flex items-center gap-3 text-[11px] uppercase tracking-wider text-slate-600">
              ...
              Trusted by leading shipyards & research institutes
              <div className="h-px flex-1 bg-white/10" />
            </div>
          </motion.div>
        </div>
...
/* ============================================
   CFD SHIP SCENE
   Shows: hull + computational mesh + flow lines
   + pressure contours + wake pattern
============================================ */
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="absolute bottom-4 left-4 z-10 rounded-md border border-white/10 bg-black/40 px-3 py-1.5 font-mono text-[10px] backdrop-blur-sm">
        <div className="text-slate-500">
          Solver: <span className="text-emerald-300">XCHAP</span>
        </div>
        <div className="text-slate-500">
          Iter: <span className="text-emerald-300">2,847</span>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 right-4 z-10 rounded-md border border-white/10 bg-black/40 px-3 py-1.5 font-mono text-[10px] backdrop-blur-sm">
        <div className="text-slate-500">Cₜ: <span className="text-amber-300">3.42e-3</span></div>
        <div className="text-slate-500">Cf: <span className="text-amber-300">1.48e-3</span></div>
      </div>

      {/* Floating ship container */}
      <motion.div
        animate={{ y: [0, -8, 0] }}
      ...
      >
        <svg
          ...
          aria-label="CFD hull simulation"
        >
          <defs>
            {/* Pressure gradient (blue→cyan→yellow→red) */}
          ...
          {/* ============ COMPUTATIONAL MESH (background grid around hull) ============ */}
          ...
          {/* ============ INCOMING FLOW LINES (streamlines) ============ */}
          <g>
            {[
              { y: 170, delay: 0 },
              { y: 195, delay: 0.3 },
          ...
          {/* ============ HULL (with pressure contour fill) ============ */}
          <g>
            {/* Hull shape - side profile of a container ship */}
            <path
            ...
            {/* Pressure contour bands on hull */}
            <g clipPath="url(#hullClip)" opacity="0.75">
              ...
              {/* Mid (low pressure - blue/cyan) */}
              <ellipse cx="200" cy="250" rx="60" ry="25" fill="#0891b2" opacity="0.4" />
              <ellipse cx="280" cy="250" rx="50" ry="25" fill="#1e40af" opacity="0.4" />

              {/* Stern (mixed - green/yellow) */}
              <ellipse cx="380" cy="255" rx="40" ry="25" fill="#10b981" opacity="0.5" />
              <ellipse cx="420" cy="255" rx="30" ry="20" fill="#f59e0b" opacity="0.4" />
            </g>

            {/* Waterline */}
            <line x1="70" y1="253" x2="445" y2="253" stroke="#ef4444" strokeWidth="1" opacity="0.5" strokeDasharray="4 2" />

            {/* Hull mesh grid overlay */}
            <g clipPath="url(#hullClip)" opacity="0.2" stroke="#06b6d4" fill="none" strokeWidth="0.4">
              {Array.from({ length: 20 }).map((_, i) => (
                <line
                  key={`hg-${i}`}
                  x1={70 + i * 20}
          ...
          {/* ============ SUPERSTRUCTURE (bridge + containers - simplified) ============ */}
          <g>
            {/* Container stacks */}
            {[130, 160, 190, 220, 250, 280, 310].map((x, i) => (
              <rect
                ...
                width="28"
                height="30"
                fill={["#0891b2", "#1e40af", "#0891b2", "#1e40af", "#0891b2", "#1e40af", "#0891b2"][i]}
                stroke="#020617"
                strokeWidth="0.8"
                opacity="0.9"
              />
            ...
            {/* Bridge */}
            <rect x="340" y="163" width="40" height="55" fill="#1e293b" stroke="#0891b2" strokeWidth="1" />
            {[168, 178, 188, 198, 208].map((y) => (
              <rect key={y} x="345" y={y} width="30" height="4" fill="#06b6d4" opacity="0.7" />
            ))}

            {/* Radar mast */}
            <line x1="360" y1="163" x2="360" y2="130" stroke="#94a3b8" strokeWidth="1.5" />
          ...
          {/* ============ VELOCITY VECTORS (arrows around hull) ============ */}
          <g stroke="#f59e0b" strokeWidth="1" fill="#f59e0b" opacity="0.7">
            {/* Bow deflection arrows */}
            {[
              { x: 55, y: 235, angle: -25 },
              { x: 55, y: 260, angle: 20 },
              { x: 60, y: 275, angle: 35 },
          ...
          <g opacity="0.7">
            ...
            {/* Transverse waves (Kelvin pattern - 19.5° angle) */}
          ...
          <g transform="translate(180, 355)">
            <text x="0" y="-2" fill="#64748b" fontSize="7" fontFamily="monospace" letterSpacing="1">
            ...
            <text x="140" y="18" textAnchor="end" fill="#94a3b8" fontSize="6" fontFamily="monospace">
              HIGH
            </text>
          </g>
        </svg>
